...
public struct StudentInformation {
    ...
    static func get(by uniqueKey: String, _ completion: @escaping ((_ objectId: String?, _ errorMessage: String?) -> Void)) {
        ...
        let request = client.createRequest(for: url, as: SuperClient.HTTPMethod.get, with: httpHeaders, with: nil)
        StudentInformation.client.createAndRunTask(for: request) { (results, error) in
            performUpdatesOnMain {
    ...
    public static func fetchStudents(completion: ((_ errorMessage: String?) -> Void)?) {
        ...
        let url = client.getURL(for: ParseClient.Constants.urlComponents, with: ParseClient.Methods.StudentLocation, with: parameters)
        let headers = [
            ParseClient.HTTPHeaderKeys.ApiKey: ParseClient.Constants.ApiKey,
            ParseClient.HTTPHeaderKeys.AppId: ParseClient.Constants.AppId
        ]
        let request = client.createRequest(for: url, as: SuperClient.HTTPMethod.get, with: headers, with: nil)
